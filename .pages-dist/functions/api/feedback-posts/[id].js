...
const FEEDBACK_MASTER_REPLY_PASSWORD = "8367";
...
async function handleD1Patch(context) {
  const db = context.env.luckystock_visitors;
  if (!db) return null;
  await ensureTable(db);
  ...
  const id = String(context.params?.id || "").trim();
  const body = await context.request.json().catch(() => ({}));
  const { masterPassword, reply } = body || {};

  if (String(masterPassword || "").trim() !== FEEDBACK_MASTER_REPLY_PASSWORD) {
    return json({ error: "invalid_password", message: "마스터 비밀번호가 맞지 않습니다." }, { status: 403 });
  }
  ...
  const replyAt = replyText ? new Date().toISOString() : "";

  await db.prepare(`
    UPDATE feedback_posts
    SET master_reply = ?1, master_reply_at = ?2
    WHERE id = ?3
  `).bind(replyText, replyAt, id).run();

  return json({
    item: serializePost({ ...existing, master_reply: replyText, master_reply_at: replyAt })
  });
}
...
export async function onRequestPatch(context) {
  const d1Response = await handleD1Patch(context);
  if (d1Response) return d1Response;
  return proxyRequest(context, "PATCH");
}
