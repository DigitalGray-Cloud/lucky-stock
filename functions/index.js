...
function json(res, code, data) {
  res.status(code).set("Cache-Control", "no-store").json(data);
}
...
export const api = onRequest({ cors: true, region: "us-central1" }, async (req, res) => {
  const url = new URL(req.originalUrl || req.url, "https://dummy.local");
  ...
  if (req.method === "GET" && url.pathname.endsWith("/news")) {
    const q = url.searchParams.get("q") || "";
    if (!q) {
    ...
    try {
      ...
      json(res, 502, { error: "failed_to_fetch_news", details: String(err?.message || err) });
    }
    return;
  }

  if (req.method === "GET" && url.pathname.endsWith("/analyze")) {
    const q = url.searchParams.get("q") || "";
    if (!q) {
      json(res, 400, { error: "q is required" });
      return;
    ...
    try {
      ...
      if (!analysis) {
        json(res, 404, { error: "stock_not_found" });
        return;
      }
      json(res, 200, analysis);
    } catch (err) {
      json(res, 500, { error: "analysis_failed", details: String(err?.message || err) });
    }
    return;
  }
