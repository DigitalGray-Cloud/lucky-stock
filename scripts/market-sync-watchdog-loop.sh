...
PID_FILE="data/market-sync-watchdog.pid"
LOG_FILE="data/market-sync-watchdog.log"
HEARTBEAT_FILE="data/market-sync-watchdog.heartbeat"
LOCK_DIR="/tmp/luckystock-market-sync-watchdog.lock"
...
while true; do
  rotate_log
  write_heartbeat
  /usr/bin/npm run batch:market-sync:ensure >> "$LOG_FILE" 2>&1 || log "ensure command failed"
  write_heartbeat
